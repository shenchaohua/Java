...
//        env.setParallelism(1);
        ...
        SingleOutputStreamOperator<JSONObject> jsonWithNewFlagDS = jsonobjDS
                ...
                .map((MapFunction<JSONObject, JSONObject>) data -> {
                    String mid = data.getJSONObject("common").getString("mid");
                    Random random = new Random();
                    data.getJSONObject("common").put("mid", mid + "-" + random.nextInt(10));
                    return data;
                })
...
//                .map(new NewMidRichMapFunc());
        ...
        // 过滤出 页面数据
        SingleOutputStreamOperator<JSONObject> pageObjDS =
                jsonWithNewFlagDS.filter(data -> StringUtils.isEmpty(data.getString("start")));
